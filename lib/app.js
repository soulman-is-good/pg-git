...
const fs = require('fs');
// commands
const createDump = require('./create');
...
const SOURCES_DIR = path.join(CWD, 'src');
const time = Date.now();
...
switch (command) {
  case "init":
    initProject(DUMP_FILE, CWD)
      .then(() => {
        console.log(`Project initialized successfully ${calcTime(time)}sec.`);
      })
      .catch(console.error);
  case "commit":
    createDump(DUMP_FILE)
      .then(() => {
...
function loadFunction(functionName) {
  if (!name || !~name.indexOf('.')) {
    throw new Error('Name should contain schema');
  }
  const [ schema, name ] = functionName.split('.');
  const sources = path.resolve(SOURCES_DIR, 'schemas', schema, 'functions', `${name}.psql`);

  if (!fs.existsSync(sources)) {
    throw new Error(`No such function ${name}`);
  }

  return fs.readFileSync(sources, { encoding: 'utf8' });
}

exports.commit = createDump;
