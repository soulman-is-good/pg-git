...
module.exports = (arch, version = '') => {
  const versionSet = version.split('.');
  let major = parseInt(versionSet[0]) || -1;
  let minor = parseInt(versionSet[1]) || -1;
  let patch = parseInt(versionSet[2]) || -1;
  let latestMajor;
  let latestMinor;
  let latestPatch;
